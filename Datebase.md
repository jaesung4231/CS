...
- **제2 정규형**<br>
  현재 테이블의 주제와 관련 없는 컬럼을 다른 테이블로 빼는 작업<br>
  제 1정규형을 만족하면서, 기본키(primary key)에 속하지 않은 속성 모두가 기본키에 완전 함수 종속인 정규형

  > **함수적 종속이란?**<br>
  > R에서 X와 Y를 각각 R의 속성 집합의 부분 집합이라고 가정하자. 속성 X의 값 각각에 대해 시간에 관계없이 항상 속성(attribute) Y의 값이 오직 하나만 연관되어 있을 때 Y는 X에 함수 종속이라 하고, X -> Y라고 표기한다.

- **제 3정규형**<br>
  종속된 컬럼은 다른 테이블로 빼기<br>
  제 2정규형을 만족하고, 테이블 내의 모든 속성이 키에만 의존하며 다른 후보키에 의존하지 않는다.

- **BCNF 정규형**<br>
  기본키가 다른 키에 종속되어 있으면 안 된다.<br>

  제 3정규형만을 만족할 경우, 이상현상들이 발생할 수 있다.

  - **삽입 이상:** 데이터를 삽입하기 위해 불필요한 데이터도 함께 삽입해야 되는 문제
  - **갱신 이상:** 중복 튜플 중 일부만 변경하여 데이터가 불일치하게 되는 문제
  - **삭제 이상:** 튜플을 삭제하면 꼭 필요한 데이터까지 같이 삭제되는 데이터 손실 문제

  이 이상현상을 해결하기 위해서 모든 결정자는 항상 후보키가 되도록 분해해주면 BCNF를 만족하게 된다
